#!/usr/bin/env python3
"""
Group the flat r3 beam-on bee/ zips into bee/chunk_NN/subchunk_MM/, the SAME
layout as the off-beam, on-beam, nuecc and numucc samples (see
chunk_onbeam_sample.py / chunk_offbeam_sample.py):
  ...
  100 zips / chunk, 10 / subchunk -- 4289 zips does NOT divide evenly, so the
...
already sitting in its computed target is left alone.
  ...
  python3 chunk_r3_beam_on_sample.py --dry-run     # report only
  python3 chunk_r3_beam_on_sample.py               # do it
"""
import argparse
import random
import re
...
BEE = Path("/Volumes/My Passport/Research_Life/Experiment/SBND/"
           "Wirecell_Reconstruction/Samples/r3-beam-on-2026-09-10/bee")
SEED = 20260910
CHUNK_SIZE = 100
SUBCHUNK_SIZE = 10
...
def main():
    ...
    with open(manifest, "w") as f:
        f.write(f"# r3 beam-on sample -- {len(rows)} zips in bee/chunk_NN/subchunk_MM/ "
