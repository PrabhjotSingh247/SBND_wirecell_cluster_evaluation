#!/usr/bin/env python3
"""
Group the flat on-beam (beam-on) bee/ zips into bee/chunk_NN/subchunk_MM/,
the SAME layout as the off-beam, nuecc and numucc samples (see
chunk_offbeam_sample.py / chunk_numucc_sample.py):

  chunk assignment : seeded random  (seed 20260830 -- the sample's own sync
                     date, same convention as off-beam's seed 20260830,
                     nuecc's seed 20260829 and numucc's seed 20260830)
  ...
  100 zips / chunk, 10 / subchunk -- 1000 zips divides evenly into 10 chunks
  x 10 subchunks x 10 files, no remainder
...
already sitting in its computed target is left alone.
  ...
  python3 chunk_onbeam_sample.py --dry-run     # report only
  python3 chunk_onbeam_sample.py               # do it
"""
import argparse
import random
import re
...
BEE = Path("/Volumes/My Passport/Research_Life/Experiment/SBND/"
           ...
           "img-clus-match-tag-pr-data-1000evt-sync-2026-08-30/beam-on/bee")
SEED = 20260830
CHUNK_SIZE = 100
SUBCHUNK_SIZE = 10
...
def main():
    ...
    with open(manifest, "w") as f:
        f.write(f"# on-beam sample -- {len(rows)} zips in bee/chunk_NN/subchunk_MM/ "
                f"({CHUNK_SIZE}/chunk, {SUBCHUNK_SIZE}/subchunk)\n")
