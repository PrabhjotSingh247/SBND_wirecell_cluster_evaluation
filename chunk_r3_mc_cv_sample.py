#!/usr/bin/env python3
"""
Group the flat r3 mc-cv (BNBLightCV, aka NuMuCC) bee/ zips into
bee/chunk_NN/subchunk_MM/, the SAME layout as the r3 nuecc, r3 beam-on, r3
beam-off, off-beam, on-beam and old numucc samples (see
chunk_r3_nuecc_sample.py / chunk_r3_beam_off_sample.py):

  chunk assignment : seeded random  (seed 20260909 -- the sample's own sync
                     date, same convention as the other samples)
  subchunk         : by SORTED zip name within the chunk, sliced into 10s --
                     which 10 files land in which subchunk is therefore random
                     (decided by the chunk-level shuffle), even though the
                     within-chunk slicing itself is deterministic by name
  100 zips / chunk, 10 / subchunk -- 1243 zips does NOT divide evenly, so the
  last chunk and its last subchunk will be short
...
already sitting in its computed target is left alone.
  ...
  python3 chunk_r3_mc_cv_sample.py --dry-run     # report only
  python3 chunk_r3_mc_cv_sample.py               # do it
"""
import argparse
import random
import re
...
BEE = Path("/Volumes/My Passport/Research_Life/Experiment/SBND/"
           "Wirecell_Reconstruction/Samples/r3-mc-cv-2026-09-09/bee")
SEED = 20260909
CHUNK_SIZE = 100
SUBCHUNK_SIZE = 10
...
def main():
    ...
    with open(manifest, "w") as f:
        f.write(f"# r3 mc-cv (BNBLightCV / NuMuCC) sample -- {len(rows)} zips in "
                f"bee/chunk_NN/subchunk_MM/ ({CHUNK_SIZE}/chunk, {SUBCHUNK_SIZE}/subchunk)\n")
