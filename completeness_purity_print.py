...
def PrintCompletenessTable(all_completeness_results, input_directories_map, output_dir):
    """
    Print and save detailed completeness tables sorted by true cluster energy.
    Saves organized in hierarchy: output_dir/input_file/event_N/completeness/
    """
    
    if len(all_completeness_results) == 0:
        print("No completeness results to print")
        return
    ...
    for event_key, event_results in results_by_event.items():
        ...
        # Prepare output text
        output_text = "\n" + "="*120 + "\n"
        output_text += f"COMPLETENESS TABLE - Event {event} ({input_file_name})\n"
        output_text += "Sorted by True Cluster Energy (descending)\n"
        output_text += "="*120 + "\n\n"
        
        # Column headers
        output_text += f"{'Event':<8} {'True ID':<12} {'Reco ID':<12} {'True Energy':<15} {'Matched Energy':<15} {'Completeness':<12} {'Status':<20}\n"
        output_text += "-"*120 + "\n"
        ...
        for idx, row in df.iterrows():
            ...
            if abs(completeness - (-0.1)) < 0.001:  # Unmatched (sentinel value)
                status = "UNMATCHED"
                zero_eff_count += 1
            elif completeness == 0:
                status = "ZERO COMPLETENESS"
                zero_eff_count += 1
            ...
            output_text += f"{event:<8} {true_id:<12.2f} {reco_id:<12.2f} {true_energy:<15.2f} {matched_energy:<15.2f} {completeness:<12.4f} {status:<20}\n"
        ...
        output_text += f"  Zero completeness: {zero_eff_count}\n"
        output_text += f"  Low completeness (<0.5): {low_eff_count}\n"
        output_text += f"  Good completeness (≥0.5): {good_eff_count}\n"
        output_text += "\n" + "="*120 + "\n"
        ...
        # Save to file
        table_file = event_output_dir / "CompletenessTable.txt"
...
def PrintCompletenessSummaryTable(all_completeness_results, output_dir):
    """
    Print and save SUMMARY completeness table aggregating across ALL files and ALL events.
    Saves to top-level output_dir (not nested in file/event subdirectories).
    """
    
    if len(all_completeness_results) == 0:
        print("No completeness results for summary table")
        return
    ...
    # Column headers
    output_text += f"{'Event':<8} {'File':<30} {'True ID':<12} {'Reco ID':<12} {'True Energy':<15} {'Matched Energy':<15} {'Completeness':<12} {'Status':<20}\n"
    output_text += "-"*130 + "\n"
    ...
    for idx, row in df.iterrows():
        ...
        output_text += f"{event:<8} {file_name:<30} {true_id:<12.2f} {reco_id:<12.2f} {true_energy:<15.2f} {matched_energy:<15.2f} {completeness:<12.4f} {status:<20}\n"
    ...
    completenesses = df['completeness_energy_weighted'].values
    ...
    output_text += f"  25th Percentile:     {np.percentile(completenesses, 25):.4f}\n"
    output_text += f"  75th Percentile:     {np.percentile(completenesses, 75):.4f}\n"
    ...
    # Save to file
    table_file = output_dir / "SUMMARY_CompletenessTable.txt"
    with open(table_file, 'w') as f:
        f.write(output_text)
