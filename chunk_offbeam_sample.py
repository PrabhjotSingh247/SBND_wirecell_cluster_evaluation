#!/usr/bin/env python3
"""
Group the flat off-beam (beam-off) bee/ zips into bee/chunk_NN/subchunk_MM/,
the SAME layout as the nuecc and numucc samples (see chunk_numucc_sample.py /
the nuecc sample's bee/chunk_manifest.txt):

  chunk assignment : seeded random  (seed 20260830 -- the sample's own sync
                     date, same convention as nuecc's seed 20260829 and
                     numucc's seed 20260830)
...
already sitting in its computed target is left alone.
  ...
  python3 chunk_offbeam_sample.py --dry-run     # report only
  python3 chunk_offbeam_sample.py               # do it
"""
import argparse
import random
import re
...
BEE = Path("/Volumes/My Passport/Research_Life/Experiment/SBND/"
           ...
           "img-clus-match-tag-pr-data-1000evt-sync-2026-08-30/beam-off/bee")
SEED = 20260830
CHUNK_SIZE = 100
SUBCHUNK_SIZE = 10
...
def main():
    ...
    with open(manifest, "w") as f:
        f.write(f"# off-beam sample -- {len(rows)} zips in bee/chunk_NN/subchunk_MM/ "
