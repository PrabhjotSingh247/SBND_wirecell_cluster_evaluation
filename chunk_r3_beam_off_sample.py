#!/usr/bin/env python3
"""
Group the flat r3 beam-off bee/ zips into bee/chunk_NN/subchunk_MM/, the SAME
layout as the r3 beam-on, off-beam, on-beam, nuecc and numucc samples (see
chunk_r3_beam_on_sample.py / chunk_onbeam_sample.py):

  chunk assignment : seeded random  (seed 20260911 -- the sample's own sync
  ...
  100 zips / chunk, 10 / subchunk -- 1088 zips does NOT divide evenly, so the
...
already sitting in its computed target is left alone.
  ...
  python3 chunk_r3_beam_off_sample.py --dry-run     # report only
  python3 chunk_r3_beam_off_sample.py               # do it
"""
import argparse
import random
import re
...
BEE = Path("/Volumes/My Passport/Research_Life/Experiment/SBND/"
           "Wirecell_Reconstruction/Samples/r3-beam-off-2026-09-11/bee")
SEED = 20260911
CHUNK_SIZE = 100
SUBCHUNK_SIZE = 10
...
def main():
    ...
    with open(manifest, "w") as f:
        f.write(f"# r3 beam-off sample -- {len(rows)} zips in bee/chunk_NN/subchunk_MM/ "
